...
cv::Mat extract_keyframe(const std::string &video_path, double timestamp)
{
    ...
    cv::Mat frame, prev_frame, min_motion_frame;
    double min_motion = std::numeric_limits<double>::max();

    cap.read(prev_frame);
    min_motion_frame = prev_frame.clone();

    for (int i = 0; i < 30 && cap.read(frame); i++)
    {
        ...
        double motion = cv::mean(magnitude)[0];

        if (motion < min_motion)
        {
            min_motion = motion;
            min_motion_frame = frame.clone();
        }
